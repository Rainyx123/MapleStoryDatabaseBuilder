...
# ...(保留原本的 SLOT_NAME_MAP 與 SLOT_ORDER)...
SLOT_NAME_MAP = {"武器": "武器", "輔助武器": "副武", "徽章": "徽章", "機器人心臟": "心臟", "帽子": "帽子", "衣服(上)": "上衣", "褲子": "褲/裙", "鞋子": "鞋子", "手套": "手套", "披風": "披風", "肩飾": "肩飾", "臉飾": "臉飾", "眼飾": "眼飾", "戒指1": "戒指1", "戒指2": "戒指2", "戒指3": "戒指3", "戒指4": "戒指4", "耳環": "耳環", "腰帶": "腰帶", "墜飾1": "墜飾1", "墜飾2": "墜飾2", "口袋物品": "口袋", "胸章": "胸章", "勳章": "勳章", "馴服的怪物": "圖騰1", "馬鞍": "圖騰2", "怪物裝備": "圖騰3", "寶玉": "寶玉"}
SLOT_ORDER = {name: i for i, name in enumerate(["武器", "副武", "徽章", "心臟", "帽子", "上衣", "褲/裙", "鞋子", "手套", "披風", "肩飾", "臉飾", "眼飾", "戒指1", "戒指2", "戒指3", "戒指4", "耳環", "腰帶", "墜飾1", "墜飾2", "口袋", "胸章", "勳章", "圖騰1", "圖騰2", "圖騰3", "寶玉"], 1)}

# =================================================================
# 2. 工具函式 & 3. 解析核心 (完全保留你原本的 fetch_data, format_chinese_number, reshape_to_matrix, process_character，請直接貼上這段原本的程式碼)
# =================================================================
def fetch_data(url):
    try:
...
def process_character(char_name):
    try:
        ...
        stat_labels = {'str': 'STR', 'dex': 'DEX', 'int': 'INT', 'luk': 'LUK','max_hp': 'HP', 'max_mp': 'MP', 'attack_power': '物攻', 'magic_power': '魔攻', 'armor': '防禦', 'speed': '移動', 'jump': '跳躍','boss_damage': 'B傷', 'damage': '總傷', 'all_stat': '全屬'}
        ...
        basic_info = [
            ...
            ["戰力", get_stat("戰鬥力")], ["傷害", get_stat("傷害", True)], ["終傷", get_stat("最終傷害", True)],
...
def main():
    ...
    for data in results:
        ...
        # 寫入各區塊資料 (不含欄位名稱)
        set_with_dataframe(ws, data['df_l'], row=1, col=1, include_column_header=False)
        
        ws.update_cell(1, 4, "【傳授技能】")
        set_with_dataframe(ws, data['df_link'], row=2, col=4, include_column_header=False)
        
        curr_row = 1 + len(data['df_link']) + 1
        ws.update_cell(curr_row, 4, "【內潛】")
        set_with_dataframe(ws, data['df_ab'], row=curr_row + 1, col=4, include_column_header=False)
        
        curr_row += (len(data['df_ab']) + 2)
        ws.update_cell(curr_row, 4, "【五轉 V-Matrix】")
        set_with_dataframe(ws, data['df_v'], row=curr_row + 1, col=4, include_column_header=False)
        
        curr_row += (len(data['df_v']) + 2)
        ws.update_cell(curr_row, 4, "【六轉 HEXA 進度】")
        set_with_dataframe(ws, data['df_h'], row=curr_row + 1, col=4, include_column_header=False)
